...
describe("Checking books", () => {
  test("[GET Positive test] Check the list of books", async() => {
    ...
    if (bookLength > 3) {
      ...
      expect(thirdBook.description).toMatch("Like it or not, JavaScript is everywhere these")
    } else {
  ...
  test("[GET Negative test] Check specific book ISBN='TEST'", async()=>{
    ...
    expect(resp.data.message).toBe("ISBN supplied is not available in Books Collection!")
  })
})
